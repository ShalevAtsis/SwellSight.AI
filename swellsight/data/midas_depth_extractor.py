...
# Import depth map storage system
from .depth_map_storage import DepthMapStorage, DepthMapMetadata
...
class MiDaSDepthExtractor:
    ...
    def __init__(self, model_name: str = "Intel/dpt-large", device: Optional[str] = None, 
        ...
        Args:
            ...
            storage_path: Path for depth map storage system (optional)
        """
        ...
        # Initialize storage system if path provided
        self.storage = None
        if storage_path:
            self.storage = DepthMapStorage(storage_path)
        
        logger.info(f"Initializing MiDaS depth extractor with model: {model_name}")
        logger.info(f"Using device: {self.device}")
        if self.storage:
            logger.info(f"Depth map storage enabled at: {storage_path}")
    ...
    def extract_depth(self, image_path: str, store_result: bool = True) -> DepthExtractionResult:
        """
        Extract depth map from a single beach camera image.
        
        Args:
            image_path: Path to the input image
            store_result: Whether to store the result in the storage system
        ...
        try:
            ...
            # Store in storage system if enabled and requested
            if self.storage and store_result:
                self._store_depth_map_result(result)
    ...
    def _store_depth_map_result(self, result: DepthExtractionResult) -> str:
        """
        Store depth extraction result in the storage system.
        
        Args:
            result: DepthExtractionResult to store
        ...
        try:
            ...
            metadata = DepthMapMetadata(
                depth_map_id="",  # Will be generated by storage system
                original_image_path=result.original_image_path,
                image_filename=image_path.name,
                midas_model_version=self.model_name,
                extraction_timestamp=result.processing_metadata['processing_timestamp'],
                ...
                storage_format=self.storage.storage_format
            )
            
            # Store depth map
            depth_map_id = self.storage.store_depth_map(result.depth_map, metadata)
            
            logger.debug(f"Stored depth map with ID: {depth_map_id}")
            return depth_map_id
            
        except Exception as e:
            logger.error(f"Failed to store depth map result: {e}")
            raise
    
    def retrieve_stored_depth_map(self, depth_map_id: str) -> Tuple[np.ndarray, DepthMapMetadata]:
        """
        Retrieve stored depth map by ID.
        ...
        return self.storage.retrieve_depth_map(depth_map_id)
    
    def find_stored_depth_maps_by_image(self, image_filename: str) -> List[DepthMapMetadata]:
        """
        Find stored depth maps by image filename.
        ...
        return self.storage.find_by_image(image_filename)
